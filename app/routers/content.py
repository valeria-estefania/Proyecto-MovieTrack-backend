...
from utils.tmdb import buscar_en_tmdb
from datetime import datetime
from services.platform_services import get_o_guardar_plataformas
from services.content_services import obtener_recomendaciones
...
def buscar_peliculas(query: str, type: str = "movie", db: Session = Depends(get_db)):
...
# Busca en nuestra base de datos
@router.get("/search/series", response_model=list[ContentResponse])
def buscar_series(query: str, type: str = "tv", db: Session = Depends(get_db)):
    resultados_locales = db.query(Content).filter(
        Content.title.ilike(f"%{query}%"),
        Content.type == type
    ).all()
...
def get_plataformas(id_content: int, db: Session = Depends(get_db)):
    ...
    return get_o_guardar_plataformas(db, content.id_content, content.tmdb_id)


@router.get("/{id_content}/recomendaciones", response_model=list[ContentResponse])
def get_recomendaciones(id_content: int, db: Session = Depends(get_db)):
    content = db.query(Content).filter(Content.id_content == id_content).first()
    if not content:
        raise HTTPException(status_code=404, detail="Contenido no encontrado")
    return obtener_recomendaciones(content.tmdb_id)
